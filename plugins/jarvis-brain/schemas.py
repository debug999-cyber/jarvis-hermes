...
BRAIN_REMEMBER = {
    ...
    "description": (
        ...
        "Одна заметка — одна мысль. Похожие заметки автоматически объединяются. Заметка зеркалится в память Hermes "
        "(fact_store); в ответе есть fact_id. " + KINDS_HINT
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
BRAIN_RECALL = {
    ...
    "description": (
        ...
        "автоматически — используй инструмент для целенаправленного поиска. Поле hermes_facts — совпадения из памяти Hermes "
        "(тот же слой, что fact_store); оценивай их через fact_feedback."
    ),
    "parameters": {
        "type": "object",
        "properties": {
