...
| `mac_type`, `mac_click` (System Events) | ввод, клики | **Hermes `computer_use`** (cua-driver, фоновой режим, без кражи курсора) | официальная фича Hermes; MIT; 22k★ trycua/cua | 🔜 1.9.1 — включаем toolset `computer_use` в конфиге, `mac_type` оставляем для простых случаев |
| `jarvis-brain` memory (`db.py`, FTS5, trust, feedback) | долговременная память | **Hermes memory providers**: Holographic (локальный SQLite FTS5 + trust/feedback — почти 1-в-1 наш дизайн), Mem0 OSS (65k★), Hindsight local (23k★) | официальные провайдеры Hermes, один активный | 🔜 1.10 — включаем `memory.provider: holographic` по умолчанию как слой фактов; наш `db.py` сужаем до того, чего у провайдеров нет (дневник, сущности с alias, `brain_history`, feedback ↔ HUD) |
| `brain_reflect` | «всё по вопросу одним вызовом» | **Hindsight** `hindsight_reflect` (наш был смоделирован с него) | 23k★, MIT, локальный режим требует LLM-ключ | 🔜 1.10 — при `memory.provider: hindsight` делегируем |
...
| `app/JarvisMenuBar.swift` | меню-бар | **Hermes Desktop** (`hermes desktop`, macOS, голос, Cmd+K, plugin SDK) | официальный клиент | 🔜 1.9.1 — пункт меню «Открыть Hermes Desktop»; своё приложение остаётся лаунчером HUD/гейтвея |
...
| `scripts/update.py` / `jarvis update` | автообновление JARVIS | **Hermes profile distributions** (`hermes profile install github.com/…`, `hermes profile update`) | официально; переносит SOUL/skills/cron/mcp/plugins, не трогает память | 🔜 1.10 — публикуем `distribution.yaml`; `jarvis update` остаётся для HUD/приложения/launchd |
...
| `jarvis_weather` | погода | Hermes бандл-навык `weather` (openclaw) / `maps` | официально | 🔜 1.9.1 — убрать свой инструмент, оставить навык |
| `screen context` (`screencapture` + `vision_analyze`) | «что на экране» | Peekaboo `see --ocr`/`--annotate` | 5.1k★ | ✅ 1.9.0 через `mac_screenshot`; 🔜 OCR в 1.9.1 |
...
_Последний полный проход: 2026-09-11 (релиз 1.9.0)._
