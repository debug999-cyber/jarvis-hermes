...
| `mac_notes` (AppleScript) | заметки | **antoniorodr/memo** — CLI из бандл-навыка Hermes `apple-notes` | 342★, Apache-2.0 | ✅ проверено в 1.10.1: `memo notes -s` — интерактивный fzf+bat TUI (`src/memo_helpers/search_memo.py`), из инструмента агента не вызывается; скриптуемы только `-v N` и `-ex` (экспорт на Desktop). Оставляем AppleScript (create/search/read, 20 строк) — честное решение «своё лучше» |
...
| `triggers.py` + `Watchdog` | события без LLM → HUD/LLM | Hermes `/heartbeat`, cron, gateway-хуки, **webhooks** (`platforms.webhook.routes`, `deliver_only`) | официальные механизмы | ✅ 1.10.1 — LLM-события идут через `hermes chat -s jarvis/heartbeat`; доставка в мессенджер — через webhook-маршрут Hermes `jarvis-alerts` с `deliver_only` и HMAC V2 (`Triggers.push`), своего транспорта к Telegram нет; локальные детекторы (батарея, диск, inbox, idle) — тонкий Mac-адаптер |
