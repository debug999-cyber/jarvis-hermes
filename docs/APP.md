...
никакого Xcode-проекта. Лежит в `~/Applications/JARVIS.app`, запускается при входе в систему (`ai.jarvis.app`).
...
**После запуска окна не появляется** — ищите ◉ справа вверху, рядом с часами. При первом запуске показывается подсказка об этом.
