...
## Принцип №1: сначала GitHub, потом свой код

Правило, установленное автором проекта (ERTGYKI) и обязательное для любого изменения:

1. **Сначала ищем готовое.** Перед тем как писать любой новый модуль/инструмент/скрипт — изучаем, что уже есть:
   встроенные возможности Hermes (`docs/llms.txt`, каталог навыков, `hermes mcp catalog`, `hermes plugins`),
   затем GitHub (звёзды, активность за последние 3 месяца, лицензия, поддержка macOS, наличие CLI/MCP).
2. **Интегрируем лучшее готовое решение.** Предпочтение: официальные механизмы Hermes → зрелые проекты с
   активным сопровождением (>300★ или официальный статус) → всё остальное.
3. **Пишем сами только если ничего достойного нет** — и тогда пишем тонко: адаптер, а не платформу.
4. Каждое такое решение фиксируем в `docs/AUDIT.md` (что заменили, чем, почему) и в `docs/SOURCES.md`.

Мерило: строки собственного кода — это долг, а не актив.
